...
// It instructs the model to evaluate its own output through four specialist lenses
// and compute a qualityScore + qualityFlags before responding.
const selfQABlock = `
<self_evaluation>
After generating the document, evaluate it as four internal specialists:
...
qualityScore = (subject_score + mece_score + style_score) / 3 - (completeness_risk * 0.2)
